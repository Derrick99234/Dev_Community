...
const signup = async (req, res) => {
    ...
    if (!email || !displayName || !username || !password || !dob) return res.status(400).json({ error: 'Input fields cannot be empty' });

    if (!validator.isEmail(email)) return res.status(400).json({ error: 'Invalid email address' });

    if (password.length < 8) return res.status(400).json({ error: 'Password length must be equal to or more than 8 characters' });
    ...
    const user = await User.create({ email, displayName, username, password: hash, dob });
